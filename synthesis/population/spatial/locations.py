...
def execute(context):
    ...
    n_missing = df_secondary_locations["geometry"].isna().sum()
    if n_missing > 0:
        print("Warning: {} secondary activities have no location (endpoint outside study area), dropping".format(n_missing))
        df_secondary_locations = df_secondary_locations.dropna(subset=["geometry"])

...
def execute(context):
    ...
    # Keep only enterprises whose commune is within the requested spatial scope.
    df_municipalities = context.stage("data.spatial.municipalities")
    requested_communes = set(df_municipalities["commune_id"].unique())
    outside = ~df["commune_id"].isin(requested_communes)
    if outside.any():
        print("Dropping %d BPE entries outside requested communes" % outside.sum())
        df = df[~outside]
